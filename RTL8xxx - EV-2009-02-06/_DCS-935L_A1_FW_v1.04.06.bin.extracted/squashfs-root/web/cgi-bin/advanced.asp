  	...
  	if ((document.getElementById("OldPassword").value == account[1]) && (document.getElementById("NewPassword").value == document.getElementById("RetypePassword").value))
  	{
		if (CheckUserpass(document.getElementById("NewPassword").value)) 
		{
     	...
     	else 
     	{
			...
			makeRequest("/cgi/admin/pwdgrp.cgi", params, modifyAdminCallback);
     	}
  	} 
  	else 
  	{
...
function DoSumitServer()
{
	...
	if (document.getElementsByName("radio_osd_time")[0].checked)
	{
  	...
  	if (parent)
		parent.blockUI(true, item_name[I_SAVING], 1);
	g_nSaving = 1;
  	makeRequest(url, params, DoSumitServer_callback);
}
function DoSumitAdd()
{	
	var iffun = document.getElementById("if_data").contentWindow.getConfig;
...
function modifyAdminCallback(http_request)
{
	if (http_request.readyState == 4) {
        if (http_request.status == 200) {
        	makeRequest("/advanced_data.asp", null, checkAuthCallback);
        }else {
            alert(pop_msg[PMSG_REQUEST_FAILED]);
        }
    }
...
function DeleteUser(userid)
{
	...
	if (parent)
		parent.blockUI(true, item_name[I_SAVING], 1);
	g_nSaving = 1;
	var params = "action=remove&grp=users&user=" + encodeURIComponent(userid);
	makeRequest("/cgi/admin/pwdgrp.cgi", params, addUserCallback);
}
...
function loadUser()
{
	...
	for (var i = 0; i < usersList.length; i++)
	{
		var num = document.getElementById("table_user").rows.length;
 		...
 		Td = Tr.insertCell(Tr.cells.length);
 		Td.width="312";
 		Td.innerHTML = "<span>" + usersList[i] + "</span>";
 		Td = Tr.insertCell(Tr.cells.length);
 		Td.width="36";
...
function init(bReinit) 
{
	...
	if (!bReinit)
	{
		...
		if (location.protocol.toLowerCase() == "https:")
		{
			...
			{
				httpport = "<% getUPnPInfo(ExternHTTPPort); %>";
			}
			if (httpport != "80")
			{
...
<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0" bgcolor="#FFFFFF" style="display: none">
  ...
  <iframe id="if_data" name="if_data" frameborder="0" width="0" height="0" src="advanced_data.asp" style="display: none"></iframe>
</body>
</html>
